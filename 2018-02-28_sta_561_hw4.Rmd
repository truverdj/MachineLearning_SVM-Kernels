...
firstPred = predict(firstSVM, newdata = test.df, decision.values = TRUE)
truth = test.df$Class
...
i = 1
for (c in seq(-5,5, length.out = numpoints)){
  decision.values = attributes(firstPred)$decision.values
  newDecision = decision.values + c
  predictions = sign(newDecision)
